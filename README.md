...
### Lessons
While working on the assignment we learnd how to use the motion sensor with the pi. We also had to realize that there is a slight delay with the sensor so it takes a few seconds after motion has stopped before the video stops recording. [Click here for the code.](https://github.com/etolton49/Engineering_4_Notebook/blob/master/Python/Parent_Detector.py)
## Push Button Stop Motion
### Objectives
For this assignment we had to take pictures using a button on the pi. From there we had to compile the photos into a video file to create a stop motion video.
### Lessons
This assignment was particulary difficult because the original package we need to compile the photos is no longer up to date and does not work. Instead of downloading libav-tools we instead download mmfpeg; however, this was also difficult because it is a large package and took an extremely long time to download. Then we also struggled to get the file to compile. It would pulll all of the photos into a file but then each would be at a rate of 0 frames per second in the video file except for one. This meant that when we tried to play the video we got one photo stuck on the screen and no others would play. Eventually we had to give up because our code was fine but the pi couldn't compile the images into a video. [Our code can be found here](https://github.com/etolton49/Engineering_4_Notebook/blob/master/Python/Stop_motion.py)
